...
Then, download the `hass-rename-entities.sh` script and make it executable:

```bash
chmod +x hass-rename-entities.sh
```
...
```bash
./hass-rename-entities.sh [options]
```
...
```bash
./hass-rename-entities.sh --dry-run --named-only
```
...
```bash
./hass-rename-entities.sh --dry-run \
  --named-only \
  -i bluetooth \
...
```bash
./hass-rename-entities.sh --dry-run -m switchbot
```
